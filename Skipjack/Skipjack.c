...
{ \
	w ^= (word16)tab[i][w & 0xff] << 8; \
	w ^= (word16)tab[j][w >>   8]; \
	w ^= (word16)tab[k][w & 0xff] << 8; \
	w ^= (word16)tab[l][w >>   8]; \
}
...
extern void makeKey(byte key[], byte tab[][256])
{
	int i;
	for (i = 0; i < 10; i++) {
		int c;
...
extern void encrypt(byte tab[][256], byte in[], byte out[])
{
	word16 w1, w2, w3, w4;
...
extern void decrypt(byte tab[][256], byte in[], byte out[])
{
	word16 w1, w2, w3, w4;
